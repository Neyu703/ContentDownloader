...
beforeEach(async () => {
  ...
  // here avoids a real 220ms setTimeout outliving a fast test and firing during a later one.
  jest
...
describe("job list rendering", () => {
  ...
  it("wires onCancel and onRetry to the downloader", async () => {
    ...
    await mockDownloader.push([makeJob({ id: "a", phase: "error" })]);
...
describe("setup phase messages", () => {
  it.each([
    ["preparing", "setup.preparing", undefined, "Bereite yt-dlp vor…"],
    ["updating", "setup.updating", undefined, "Suche nach yt-dlp-Update…"],
    ["failed", "errors.raw", { raw: "USB nicht bereit" }, "USB nicht bereit"],
  ] as const)(
    "shows the translated setup message while phase is %s",
    async (phase, messageKey, messageParams, expectedText) => {
      await render(<HomeScreen />);
      await mockDownloader.push([], { phase, message: messageKey, messageParams });
  ...
  it("shows nothing extra once setup is ready", async () => {
    ...
    expect(screen.queryByText(/Bereite|Suche nach|nicht bereit/)).toBeNull();
  });
});
